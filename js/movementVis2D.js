...
function drawFlowsByYear(year, checkboxStatue) {
    ...
    var routes = d3.nest().key(function (d) {
        return d.year;
    }).sortKeys(d3.ascending)
        .entries(dataShipment);

    var keyArr = routes.map(function (d) {return d.key;});

    var index = keyArr.indexOf(year);

    //console.log("year: ",year ,routes[index]);
    ...
    //drawGraphic(routes[index].values);
